    ...
    for x in text:
        ...
        if not regex.match(x) and not regex4.match(x) and not regex3.match(x) and  not regex2.match(x):
            ...
            if css not in CSS:
                ...
                x = ''.join(ch for ch in x if ch =='-' or ch not in exclude)
                #ignore case
                x = x.lower()
                #ignora le stopwords o le parole di 1 o 2 caratteri
                if x not in cachedStopWords and not x.startswith('url') and len(x) >2:
                    toret.append(x)
    return toret
...
def invert(db,url):
    ...
    for word in db[url]:
        ...
        inverted_db[word][url] = float(count)/ len(db[url])
