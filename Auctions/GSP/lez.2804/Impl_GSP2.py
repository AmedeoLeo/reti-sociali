...
def best_response(name, adv_value, slot_ctrs, history):
    ...
    #2) Evaluate for each slot, which one gives to the advertiser the largest utility
        ...
        print "\t\t new_utility: %.20f" % new_utility
        print "\t\t utility: %.20f" % utility
        if new_utility > utility:
            utility = new_utility
            preferred_slot = i
            print "\t\tpreferred_slot: %d" % preferred_slot
            payment = tmp_pay
    ...
    print "adv_value: %.24f" % adv_value
    if preferred_slot == -1:
        # TIE-BREAKING RULE: I choose the largest bid smaller than my value for which I lose
        bid = min(adv_value, sort_bids[len(sort_slots)])
        print "bid for %s : %.20f" % (name, bid)
    ...
    bid = (adv_value - float(slot_ctrs[sort_slots[preferred_slot]])/slot_ctrs[sort_slots[preferred_slot-1]] * (adv_value - payment))
    print "bid for %s : %.20f" % (name, bid)
