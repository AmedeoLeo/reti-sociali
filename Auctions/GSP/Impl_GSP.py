...
def best_response(name, adv_value, slot_ctrs, history):
    ...
    curr_utility = (adv_value - tmp_pay) * slot_ctrs[sort_slot[i]]
    ...
    #ToDo
    """Casi limite:
	   1. Restituisce la bid minima tra del primo giocatore che non vince oppure il mio valore stesso
	   2. Restituisce un numero a metà tra la mia valutazione e quella del successivo"""
    if best_slot == -1:
		return min(adv_value, sort_bids[len(sort_bids)])
	if best_slot == 0:
		return float(adv_value + payment)/2
	"""
	L'utilità attuale deve essere la stessa a quella nel caso in cui prendo la slot precedente pagando esattamente la mia valutazione
	(i due casi sono indifferenti):
	(adv_value - tmp_pay) * slot_ctrs[sort_slot[best_slot]] = 	(adv_value - bid) * slot_ctrs[sort_slot[best_slot-1]]
	"""
	return (adv_value - float(slot_ctrs[sort_slot[best_slot]] / slot_ctrs[sort_slot[best_slot-1]]*(adv_value - payment))
