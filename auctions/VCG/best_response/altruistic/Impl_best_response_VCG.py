...
def best_response(name, adv_value, slot_ctrs, history):
    ...
    temp =  sorted(adv_bids, key=lambda x:adv_bids[x], reverse=True)
    sorted_name = od((x, adv_bids[x]) for x in temp)
    ...
    #2) Evaluate for each slot, which one gives to the advertiser the largest utility
        ...
        if new_utility > utility:
            print >> output,  "vecchia utility ", utility,  " nuova utility ",  new_utility
            utility = new_utility
            preferred_slot = i
            payment = tmp_pay
    ...
    if preferred_slot == 0:
        ...
        if sorted_name.keys()[0] == name:
            print >> output,"Rule 2 - winner: new bid: ", float(adv_value+payment)/2
            return float(adv_value+payment)/2
        else:
            print >> output,"Rule 2 - no winner: new bid: ", float(sort_bids[0] +epsilon)
            return float(sort_bids[0] +epsilon)
    ...
    print >> output,"Rule 3: new bid: ",  float(sort_bids[preferred_slot] +epsilon),  "per slot ", preferred_slot
    print step,": ", name,  " ",  preferred_slot, " ", sort_bids[preferred_slot] 
    return float(sort_bids[preferred_slot] +epsilon)
